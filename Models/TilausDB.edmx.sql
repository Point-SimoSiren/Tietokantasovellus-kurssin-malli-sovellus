...
-- --------------------------------------------------
-- Date Created: 06/07/2021 10:08:36
...
IF OBJECT_ID(N'[dbo].[FK_Asiakkaat_Post]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Asiakkaat] DROP CONSTRAINT [FK_Asiakkaat_Post];
GO
...
GO
IF OBJECT_ID(N'[dbo].[Testi]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Testi];
GO
...
-- Creating table 'Testi'
CREATE TABLE [dbo].[Testi] (
    [TestiId] int  NOT NULL,
    [Nimi] nvarchar(50)  NULL,
    [Jotain] nvarchar(50)  NULL
);
GO
...
-- Creating primary key on [TestiId] in table 'Testi'
ALTER TABLE [dbo].[Testi]
ADD CONSTRAINT [PK_Testi]
    PRIMARY KEY CLUSTERED ([TestiId] ASC);
GO
...
-- Creating foreign key on [Postnro] in table 'Asiakkaat'
ALTER TABLE [dbo].[Asiakkaat]
ADD CONSTRAINT [FK_Asiakkaat_Post]
    FOREIGN KEY ([Postnro])
    REFERENCES [dbo].[Post]
        ([Postnro])
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Asiakkaat_Post'
CREATE INDEX [IX_FK_Asiakkaat_Post]
ON [dbo].[Asiakkaat]
    ([Postnro]);
GO
